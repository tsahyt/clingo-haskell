...
foreign import ccall "clingo.h clingo_control_new" controlNewFFI ::
    Ptr CString -> CSize -> FunPtr (Logger a) -> Ptr a -> CUInt -> Ptr (Ptr Control) -> IO CBool
foreign import ccall "clingo.h clingo_control_free" controlFreeFFI ::
    Ptr Control -> IO ()
foreign import ccall "clingo.h clingo_control_load" controlLoadFFI ::
    Ptr Control -> CString -> IO CBool
foreign import ccall "clingo.h clingo_control_add" controlAddFFI ::
    Ptr Control -> CString -> Ptr CString -> CSize -> CString -> IO CBool
foreign import ccall "clingo.h clingo_control_ground" controlGroundFFI ::
    Ptr Control -> Ptr Part -> CSize -> FunPtr (CallbackGround a) -> Ptr a -> IO CBool
foreign import ccall "clingo.h clingo_control_solve" controlSolveFFI ::
    Ptr Control -> FunPtr (CallbackModel a) -> Ptr a -> Ptr SymbolicLiteral -> CSize -> Ptr SolveResult -> IO CBool
foreign import ccall "clingo.h clingo_control_solve_iteratively" controlSolveIterFFI ::
    Ptr Control -> Ptr SymbolicLiteral -> CSize -> Ptr (Ptr IterSolver) -> IO CBool
foreign import ccall "clingo.h clingo_control_solve_async" controlSolveAsyncFFI ::
    Ptr Control -> FunPtr (CallbackModel a) -> Ptr a -> FunPtr (CallbackFinish b) -> Ptr b -> Ptr SymbolicLiteral -> CSize -> Ptr (Ptr AsyncSolver) -> IO CBool
foreign import ccall "clingo.h clingo_control_cleanup" controlCleanupFFI ::
    Ptr Control -> IO CBool
foreign import ccall "clingo.h clingo_control_assign_external" controlAssignExternalFFI ::
...
foreign import ccall "clingo.h clingo_control_interrupt" controlInterruptFFI ::
    Ptr Control -> IO ()
foreign import ccall "clingo.h clingo_control_clasp_facade" controlClaspFacadeFFI ::
    Ptr Control -> Ptr (Ptr ()) -> IO CBool
foreign import ccall "clingo.h clingo_control_configuration" controlConfigurationFFI ::
    Ptr Control -> Ptr (Ptr Configuration) -> IO CBool
foreign import ccall "clingo.h clingo_control_use_enumeration_assumption" controlUseEnumAssumptionFFI ::
    Ptr Control -> CBool -> IO CBool
foreign import ccall "clingo.h clingo_control_get_const" controlGetConstFFI ::
    Ptr Control -> CString -> Ptr Symbol -> IO CBool
foreign import ccall "clingo.h clingo_control_has_const" controlHasConstFFI ::
    Ptr Control -> CString -> Ptr CBool -> IO CBool
foreign import ccall "clingo.h clingo_control_symbolic_atoms" controlSymbolicAtomsFFI ::
...
controlNew :: MonadIO m => Ptr CString -> CSize -> FunPtr (Logger a) -> Ptr a -> CUInt -> Ptr (Ptr Control) -> m CBool
...
controlAdd :: MonadIO m => Ptr Control -> CString -> Ptr CString -> CSize -> CString -> m CBool
controlAdd a b c d e = liftIO $ controlAddFFI a b c d e

controlGround :: MonadIO m => Ptr Control -> Ptr Part -> CSize -> FunPtr (CallbackGround a) -> Ptr a -> m CBool
controlGround a b c d e = liftIO $ controlGroundFFI a b c d e

controlSolve :: MonadIO m => Ptr Control -> FunPtr (CallbackModel a) -> Ptr a -> Ptr SymbolicLiteral -> CSize -> Ptr SolveResult -> m CBool
controlSolve a b c d e f = liftIO $ controlSolveFFI a b c d e f

controlSolveIter :: MonadIO m => Ptr Control -> Ptr SymbolicLiteral -> CSize -> Ptr (Ptr IterSolver) -> m CBool
controlSolveIter a b c d = liftIO $ controlSolveIterFFI a b c d

controlSolveAsync :: MonadIO m => Ptr Control -> FunPtr (CallbackModel a) -> Ptr a -> FunPtr (CallbackFinish b) -> Ptr b -> Ptr SymbolicLiteral -> CSize -> Ptr (Ptr AsyncSolver) -> m CBool
controlSolveAsync a b c d e f g h = liftIO $ controlSolveAsyncFFI a b c d e f g h

controlCleanup :: MonadIO m => Ptr Control -> m CBool
controlCleanup a = liftIO $ controlCleanupFFI a

controlAssignExternal :: MonadIO m => Ptr Control -> Symbol -> TruthValue -> m CBool
...
controlConfiguration :: MonadIO m => Ptr Control -> Ptr (Ptr Configuration) -> m CBool
...
controlSymbolicAtoms :: MonadIO m => Ptr Control -> Ptr (Ptr SymbolicAtoms) -> m CBool
...
controlProgramBuilder :: MonadIO m => Ptr Control -> Ptr (Ptr ProgramBuilder) -> m CBool
